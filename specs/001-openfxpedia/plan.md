...
-## Constitution Check
- Repository conventions: specs under `specs/001-openfxpedia` (matches existing layout)
...
- Image sourcing: use generic icons bundled with the app (no runtime fetching). This simplifies licensing and offline UX.
- Supported currency set: include all non-deprecated ISO 4217 currencies from the CDN JSON; UI will lazily load/paginate lists.
- Rate refresh policy: refresh on app open, background refresh every 12 hours, and manual refresh available.
...
- Project layout suggestion:
	...
	- `specs/001-openfxpedia/` — research, data-model, contracts, tasks
